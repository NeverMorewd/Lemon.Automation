...
class Debugger {
    ...
    async _attach() {
        try {
            try {
                await chrome.debugger.attach({ tabId: this.tabId }, '1.3', null);
            }
            catch (e) {
    ...
    _sendCommand(method, commandParam) {
        return chrome.debugger.sendCommand({ tabId: this.tabId }, method, commandParam);
                ...
                case "init":
                    try 
                    {
                        ...
                        chrome.windows.getCurrent(function (currentWindow) 
                        {
                            ...
                            {
                                const result = debuggerManager.sendCommand(activeTabs[0].id,'Runtime.evaluate',{expression: message.params.code })
                                result.then(function (value)
                                {
                                    console.log('evaluate return:');
                                    console.log(value);
                                });
                                console.log(result);
                            });
                        });
                        //eval.call(window, message.params.code)
